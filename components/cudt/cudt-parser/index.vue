<template>
	<u-form :model="formData" :rules="rules" ref="dataForm" :errorType="['toast']"
		...
		:label-align="formConf.labelPosition==='right'?'right':'left'" class="cudt-form">
		...
		methods: {
			initFormData(componentList, formData, tableRefs) {
				componentList.forEach(cur => {
					...
					if (config.children && cur.__config__.cudtKey !== 'table') {
						this.initFormData(config.children, formData)
					}
					if (cur.__config__.cudtKey == 'table' && !cur.__config__.noShow) {
			...
			buildOptions(componentList) {
				componentList.forEach(cur => {
					...
					if (config.children && config.cudtKey !== 'table') this.buildOptions(config.children)
				})
			},
			buildRules(componentList, rules) {
				componentList.forEach(cur => {
					const config = cur.__config__
					const cudtKey = config.cudtKey
					...
					const rule = config.regList.map(item => {
						...
						if (useNumList.includes(cudtKey)) item.type = 'number'
						if (Array.isArray(config.defaultValue)) item.type = 'array'
						return item
					})
					if (rule.length) rules[cur.__vModel__] = rule
					if (config.children && cudtKey !== 'table') this.buildRules(config.children, rules)
				})
			},
			setValue(item) {
