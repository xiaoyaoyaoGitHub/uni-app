	...
	<view class="flowLaunch-v">
		...
		<u-popup v-model="popupShow" mode="bottom" height="60%">
			<view style="margin: 10px 20px 10px 20px">
				<u-form ref="uForm">
					<u-form-item label="审核状态" label-position="top">
						<cudt-checkbox v-model="listQuery.auditStatus" :options="queryConditionData.auditList">
						</cudt-checkbox>
					</u-form-item>
					...
					<u-form-item label="行业" label-position="left" label-width="130rpx">
						<cudt-select v-model="listQuery.business" placeholder="请选择"
							:options="queryConditionData.businessList">
						</cudt-select>
					</u-form-item>
		...
		<scroll-view scroll-x="true" style="width: 100%;overflow: hidden;white-space: nowrap;">
			<view v-if="reportType === 0">
				<u-table v-if="listQuery.way ===1">
					<u-tr class="u-tr" />
					<u-th class="u-th" v-for="(item, index) in tableHead" :prop="item.id" :key="index">{{item.name}}
					</u-th>
					</u-tr>
				...
				<u-table v-if="listQuery.way ===2">
					<u-tr class="u-tr">
						<u-th width="30%" class="u-th" v-for="(item, index) in tableHead" :prop=item.id :key=index>
							{{item.name}}</u-th>
					</u-tr>
					<u-tr class="u-tr" v-for="(item, index) in list" :key=index>
						<u-td width="30%" class="u-td" v-for="value in tableHead" :key=value.id>{{item[value.id]}}
						</u-td>
					</u-tr>
				</u-table>
				<u-table v-if="listQuery.way ===3">
					<u-tr class="u-tr">
						<u-th width="50%" class="u-th" v-for="(item, index) in tableHead" :prop=item.id :key=index>
							{{item.name}}</u-th>
					</u-tr>
					<u-tr class="u-tr" v-for="(item, index) in list" :key=index>
						<u-td width="50%" class="u-td" v-for="value in tableHead" :key=value.id>{{item[value.id]}}
						</u-td>
					</u-tr>
				</u-table>
			</view>
			<view v-if="reportType === 1 && detailList.length  > 0">
				<u-table>
					<u-tr class="u-tr">
						<u-th width="30%" class="u-th" v-for="(item, index) in detailTableHead" :prop=item.prop
							:key=index>{{item.label}}</u-th>
					</u-tr>
					<u-tr class="u-tr" v-for="(item, index) in detailList" :key=index>
						<u-td width="30%" class="u-td" v-for="value in detailTableHead" :key=value.propF>
							{{item[value.prop]}}</u-td>
					</u-tr>
				</u-table>
			</view>
		</scroll-view>
	...
	export default {
		...
		data() {
			return {
				...
				projectPhase: '05726e181e7147768f67ce1905fe6e49', //储备项目id
