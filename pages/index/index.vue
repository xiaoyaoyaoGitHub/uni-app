<template>
	<view class="index-v">
		<mescroll-body ref="mescrollRef" @down="downCallback" :sticky="true" :up="upOption" :bottombar="false">
			<view class="banner">
				<u-swiper :list="bannerList" height="300" :title="true" :title-style="{'font-size':'20px'}"></u-swiper>
			</view>
			<u-divider fontSize="30" height="60" half-width="150">工作台</u-divider>
			<view>
			...
			<u-divider half-width="150"></u-divider>
			<view class="replyList">
				<view class="reply-item u-border-bottom u-flex">
					...
					<view class="reply-item-txt u-flex-1">
						<view class="reply-item-cell reply-item-title u-flex">
							<text class="title">通知公告</text>
							<u-badge style="float: left; margin-top:-15rpx;" type="error" :count="notificationCount" :absolute="false"/>
						</view>
					</view>
				</view>
				<template>
					<uni-list>
						<view v-for="(item, index) in list" :key="index">
							<uni-list-item :title="item.subject" :note="item.created_date" show-arrow="true" @click="toNotificationDetail(item)" link>
							</uni-list-item>
						</view>
					</uni-list>
				</template>
...
<script>
	...
	import { moduleCodes } from '@/api/common'
	export default {
						...
						uni.request({
							...
							success: res => {
								if (res.statusCode === 200) {
									banner.image = `data:image/jpeg;base64,${uni.arrayBufferToBase64(res.data)}`
			...
			loadNotification() {
				...
				getModelListViaCode(moduleCodes.notification, query).then(res => {
					...
					for(let item of res.data.list) {
						if(item.status === '已发布' && (!item.department || item.department === this.userInfo.departmentName)) {
							this.list.push(item)
						}
					}
